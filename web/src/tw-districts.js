/* 台灣縣市 → 鄉鎮市區 對照（22 縣市 · 368 區）
 * 用途：個人資料「所在地」改用兩層下拉選單（縣市→區），iPhone 原生滾輪、長輩好按、零錯字。
 * 顯示採常用「台」字（台北/台中/台南/台東），與 App 既有用字一致。
 * 2026-07-09 Edward 拍板「改用選單」後建。 */
window.TW_DISTRICTS = {
...
/* 2026-07-31：併進跨國行政區註冊表（MUNEA_REGIONS）。舊的 window.TW_DISTRICTS 保留不動，
 * 既有程式與舊資料相容照走；新的多國選單一律讀註冊表。
 * coords＝縣市政府所在地，給天氣／空氣品質用（既有台灣線走中央氣象署，coords 是境外備援）。 */
window.MUNEA_REGIONS = window.MUNEA_REGIONS || {};
window.MUNEA_REGIONS.TW = {
  countryCode: 'TW',
  tier1Key: 'profile.regionTier1County',
  tier2Key: 'profile.regionTier2District',
  otherKey: 'profile.regionOther',
  regions: (function () {
    var COORDS = {
      "台北市": [25.033, 121.565], "新北市": [25.017, 121.463], "基隆市": [25.128, 121.742],
      ...
      "嘉義縣": [23.452, 120.256], "台南市": [22.999, 120.227], "高雄市": [22.627, 120.301],
      "屏東縣": [22.552, 120.549], "宜蘭縣": [24.702, 121.738], "花蓮縣": [23.977, 121.604],
      "台東縣": [22.758, 121.144], "澎湖縣": [23.566, 119.579], "金門縣": [24.433, 118.317],
      "連江縣": [26.160, 119.951]
    };
    var out = {};
    for (var k in window.TW_DISTRICTS) {
      out[k] = { coords: COORDS[k] || null, cities: window.TW_DISTRICTS[k] };
    }
    return out;
  })(),
};
