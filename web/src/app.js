...
const LiveVoice = {
  ...
  _armDeadLineWatch(phase, waitMs) {
    ...
    this._deadLineWatchT = setTimeout(() => {
      ...
      // 2026-08-18：這裡原本寫成「上行有封包 **或** 她已經出過聲」就算線活著。
      // 但她每通都會出聲，等於這道保險絲永遠不會響——8/13 之後每一通都是
      // in_bytes=0（你的麥克風一格都沒送到伺服器），她照樣自己講了 23 秒 ×3 輪，
      // 而這道本來該喊「你的聲音沒送出去」的看門，因為她有出聲就閉嘴了。
      // 上面那段註解其實已經寫對了：**唯一能證明線活著的是上行**。程式跟著改。
      // 她的聲音只證明「她那邊好」，不證明「你講得出去」——兩件事不能互相頂替。
      const lineAlive = phase === 'ready_timeout'
        ? !!this.ready
        : (this._micPackets || 0) > 0;
      if (lineAlive) return;
