...
function updateAuthUI() {
  ...
  const metadata = state && state.user && state.user.user_metadata ? state.user.user_metadata : {};
  if (status) status.textContent = signedIn ? (state.developerMode ? (metadata.name || '測試帳號') : '已登入') : '訪客模式';
