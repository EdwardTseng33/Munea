...
|---|---|---|---|---|
| App source lane | `1.0.46 (Build 1)`；exact GitHub commit `e40dbd7`，Archive／IPA／Edward iPhone 回讀已成立 | App Store latest uploaded／selected 仍是 1.0.45；1.0.46 upload 與 exact-Build 真人 Gate 未完成 | `partial` | 精確 binary 上傳核准 → upload／processing／選取 → 真人 Gate |
