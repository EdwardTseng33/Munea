...
|---|---|---|---|---|
| App source lane | `1.0.45 (Build 500)`；純 source 升版、尚未打包，Build number 跳到 500（492 之後另有包版，實際上傳號待確認） | Build 492 的 Archive／IPA、Apple upload 與 Edward iPhone 安裝／啟動證據只適用 `72a0bd46`，不適用 1.0.45；App E2E pending | `partial` | 若驗現有 492，須針對該凍結成品；1.0.45 要出貨得先 Archive Build 500 再完整驗收 |
