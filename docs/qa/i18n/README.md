...
- 431 個 key 在繁中、英文、日文、西班牙文都存在且非空白。
- 英文與西班牙文沒有中文字殘留。
...
執行 `node scripts/i18n-native-review-worklist.js --locale ja > <工作檔>.json` 可產生單一語系的 431 筆工作檔。審稿者必須逐筆把 `result` 改為 `pass`，並把六個 `checks` 全部改為布林值 `true`；若翻譯與繁中來源完全相同且未在日文共用詞 allowlist，還必須留下 `reviewerNote`。工作檔根節點另需填入：
