...
- **長通話與畫質**：不再為追趕聲音刪除嘴型影格；生成前替完整模型批次保留佇列空間。模型仍以原生 512 推論，傳送端升頻至 640 並輕度銳化；App 下緣羽化提前收掉動態領口。候選 signed playout offset 為 `−280ms`。
- **自動假手機證據**：單路 6 輪在 `−80ms` 已證明無累積漂移；比照正式兩槽滿載後發現聲嘴 `−0.31～−0.45s`，因此未放行。改為 `−280ms` 後，雙路同時各 3 輪收斂到 `−0.17～−0.23s`，再以單路 3 輪確認 `−0.02～0.00s`；所有測試皆 640×640、video trim 0、audio underrun 0，兩槽 generation p95 616.4／710.4ms。
