...
## 2026-08-12 台灣｜1.0.65 聲音身分復原候選（server-only、0% 候選待 Gate）
...
- **候選範圍／預期體驗**：最新程式碼保留 1.0.65 已改善的連續語音，部署入口恢復核准的 `Gemini 3.1 Flash Live + Despina`；App 的 service identity 同時記錄 engine、model、voice name，往後不能只靠聲音名稱宣稱聲音未變。這是 server-only 變更，不需要重新包 App。
- **放行邊界**：先建最新程式 0% Voice 候選並通過多輪真 PCM／Avatar Gate；正式切流後仍不得把開場、512 畫質、動作硬切或真機聲嘴列為已修。這四項另案處理，exact-build App 體驗仍以使用者現有失敗證據為準。
