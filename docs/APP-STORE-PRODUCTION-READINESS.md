...
|---|---|
| App 版本 | `1.0.5` |
| Build | Xcode 原始碼已升 `11`；最新實體包仍是已作廢的 Build 10 |
| Bundle ID | `net.munea.app` |
| 候選 IPA | 無；Build 10 不含本輪上架收尾，需由乾淨 `main` 重建 Build 11 |
| iOS 最低版本 | iOS 15 |
...
- ✅ PASS：以 `main@8dfb91f` 為 App 功能來源，PR #29 點數週期、PR #23 語音開場緩衝／台語安全閘、PR #30 訂閱頁水彩新設計、PR #31 Gateway 長駐監控、PR #33 訂閱頁捲動／說明卡及 PR #34「一家人」文案已和既有帳號、Call Control、登入及拍照修正同步；Web 與 iOS 內嵌資源一致。
...
- ✅ PASS：Build 10 Release Archive 與 App Store IPA 已依 `main@8dfb91f` 重建；正式 IPA 沒有測試帳號、假資料、自動登入或開發直連，並驗證簽章、版本／Build、Bundle ID、相機／照片用途說明、HealthKit、Apple 登入 entitlement、訂閱頁 HTML／樣式／插圖、PR #33／#34 文案與最新設計資源。
...
- ⚠️ REBUILD：Xcode Debug／Release 已設定為 `1.0.5 (Build 11)`；合併後必須由乾淨 `main` 實際 Archive／匯出，並以 Xcode Privacy Report 複核第三方 SDK manifest。現有 Build 10 IPA 已過期。
...
- ❌ FAIL：Build 11 尚未由 Mac Archive／匯出，也尚未上傳 App Store Connect。

## 上傳決策

目前 **不可送審**。舊 Build 10 的簽章與 IPA 曾通過，但已因本輪原生／後端變更作廢；Build 11 尚未產包，且公開登入設定、真實登入／Gateway 通話、Cloud Run 點數對應、Sandbox 金流及資料權利仍有未通過項目。

## 下一步

1. 合併本輪上架閉環後部署 Brain canary，設定 App Store Notifications V2 URL，完成 TEST＋Sandbox 續訂／到期／退款與登入者個資匯出 E2E；通過再升 100% 流量。
2. 補齊 Google OAuth Logo／首頁／隱私權／條款並送品牌驗證；以真實 Google／Apple 帳號完成登入、登出與重登。
3. 以 canary 部署 Cloud Run 點數對應與 policy v3，驗證後再升 100% 流量。
4. 使用 Mac iPhone 鏡像完成一般畫面流程；拍照、麥克風、Face ID、通話與需要實體確認的金流則直接在 Edward iPhone 完成。
5. 完成資料匯出與帳號刪除正式帳號端到端測試。
6. 從乾淨 `main` 產出 Build 11+，生成 Xcode Privacy Report，重新驗證簽章／manifest／最新 Web 資源後上傳 App Store Connect。
7. 補完 4 個訂閱商品元資料、掛入 IAP／訂閱、App Privacy、年齡分級與審核備註後送 TestFlight。
