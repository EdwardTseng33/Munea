...
| 候選 IPA | ✅ 已匯出；58,864,007 bytes，SHA-256 `fc3a9a593291793854aefa91c202e58116bcecf1d917d3e001cf77da1287a71a` |
| 候選來源 | ✅ PR #96；基於 `main@2c72057`，完整承接 PR #94、#95 與前版 PR #92；#95 為 Voice 原始碼，不改已上傳 App 二進位 |
| App Store Connect | ✅ 2026-07-16 02:58（Asia/Taipei）上傳成功；Apple 處理中 |
| iOS 最低版本 | iOS 15 |
...
- ✅ PASS：候選來源基於 `main@2c72057`，包含通知中心、權限同步、家庭狀態、角色呼吸、個人行程、App 收音守門與 PR #94 撥號前三線暖機。
- ✅ PASS：固定 PCM 全語音探測通過台灣繁中 ASR（園藝／回診、吃藥／爸爸／血壓、阿宏／懷舊老歌／看診）、S2S 回覆、插話 7/7、30 秒低噪音不誤接話及句尾靜音保護；探測不依賴文字輸入。
- ✅ PASS：`1.0.24 (Build 30)` 開發版已覆蓋安裝到 Edward iPhone 15 Pro；手機回讀版本與啟動通過，開發 profile 設定為 TEST、Pro、1,000 點與家人假資料。
- ✅ PASS：正式 App 強制走 Call Control；只有腳本產生的 Edward 開發包可直連，Call Control 10 項契約與 Release 防漏均通過。
- ✅ PASS：Build 30 Release Archive 與 App Store IPA 已在 Mac 重建；正式 IPA 沒有測試帳號、假資料、自動登入或開發直連，並驗證 production APNs、`get-task-allow=false`、簽章、版本／Build、Bundle ID、`UIDeviceFamily=[1]`、相機／照片用途說明、HealthKit、Apple 登入 entitlement、Privacy Manifest、東京設定與最新 Web 資源。
- ✅ PASS：Build 30 已由 Xcode 正式上傳 App Store Connect；Apple 回執為 `Upload succeeded`，目前等待後台處理完成。
...
- ❌ FAIL：Build 30 的語音契約與固定探測通過，但 Edward 尚未完成真人 10 分鐘長聊，不能宣稱斷字已解決。
- ❌ FAIL：Build 30 的低噪音／插話契約通過，但 Edward 尚未完成真人五組靜音與五次插話；不能以自動結果取代真機 PASS。
- ❌ DEPLOY：PR #92／#95 的 Voice 伺服器人格、上一通記憶防編造、行程工具、稱呼頻率與純語音現實邊界尚未部署；本次 App 包版不會讓伺服器改動自動生效。
...
- ❌ FAIL：Apple 公開網頁 OAuth 仍回 `Invalid client id or web redirect url`；Build 30 使用原生 Apple 登入，但尚未以真帳號完成 ID token 端到端驗收。
...
- ❌ FAIL：Build 30 尚未由 Edward 實際點「拍照」確認相機畫面可開啟；自動檢查與權限字串已通過，但不能代替實體相機操作。
...
- ❌ WAIT：Build 30 已上傳 App Store Connect，但尚未確認 Apple 處理完成、TestFlight 可選用，也尚未產生 Xcode Privacy Report。
- ❌ FAIL：App Store Connect 的 App Privacy 尚未填完；Munea 涉及帳號、健康、音訊與使用資料，不能選「不收集資料」。
- ❌ WAIT：必須在 Apple 處理完成後選取 Build 30，才能確認 13 吋 iPad 截圖要求已消失；舊 Build 的裝置支援不會被修改。

## 上傳決策

目前 **已上傳、不可送審**。Build 30 的工程驗證、正式簽章、iPhone-only IPA 防漏與 App Store Connect 收件均通過；但 Apple 尚在處理，而且 Voice 伺服器新版、真人語音、真實登入／Gateway 通話、APNs、拍照、Sandbox 金流、App Privacy、資料權利與審核元資料仍有未通過項目。
...
5. 等待 Apple 完成 Build 30 處理，在版本頁選取新 Build，確認 iPad 截圖要求消失與 TestFlight 可選用；生成 Xcode Privacy Report 並複核第三方 SDK manifest。
