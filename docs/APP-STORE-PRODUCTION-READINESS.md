...
| 候選 IPA | ✅ 已匯出；55,539,118 bytes，SHA-256 `ddc46e5c70a42863bbab6a8ab4922ee436666740f2bb89600795bd476a8347e6` |
| 候選來源 | ✅ `main@9414234`；包含 PR #82 聊聊穩定／診斷與 PR #83 通知中心 |
| App Store Connect | ✅ 2026-07-16 00:42（Asia/Taipei）上傳成功；Apple 處理中 |
| iOS 最低版本 | iOS 15 |
...
- ✅ PASS：候選來源為最新 `main@9414234`，包含既有 App Store Server Notifications V2、本人資料匯出、Privacy Manifest，以及 PR #82 聊聊穩定／診斷與 PR #83 通知中心。
- ✅ PASS：固定 PCM 全語音探測通過台灣繁中 ASR（園藝／回診、吃藥／爸爸／血壓、阿宏／懷舊老歌／看診）、S2S 回覆、插話 7/7、30 秒低噪音不誤接話及句尾靜音保護；探測不依賴文字輸入。
- ✅ PASS：`1.0.22 (Build 27)` 開發版已覆蓋安裝到 Edward iPhone 15 Pro；手機回讀版本與啟動通過，開發 profile 設定為 TEST、Pro、1,000 點與家人假資料。
- ✅ PASS：正式 App 強制走 Call Control；只有腳本產生的 Edward 開發包可直連，Call Control 10 項契約與 Release 防漏均通過。
- ✅ PASS：Build 27 Release Archive 與 App Store IPA 已在 Mac 重建；正式 IPA 沒有測試帳號、假資料、自動登入或開發直連，並驗證 production APNs、`get-task-allow=false`、簽章、版本／Build、Bundle ID、相機／照片用途說明、HealthKit、Apple 登入 entitlement、Privacy Manifest、東京設定與最新 Web 資源。
- ✅ PASS：Build 27 已由 Xcode 正式上傳 App Store Connect；Apple 回執為 `Upload succeeded`，目前等待後台處理完成。
...
- ❌ FAIL：Build 27 的語音契約與固定探測通過，但 Edward 尚未完成真人 10 分鐘長聊，不能宣稱斷字已解決。
- ❌ FAIL：Build 27 的低噪音／插話契約通過，但 Edward 尚未完成真人五組靜音與五次插話；不能以自動結果取代真機 PASS。
...
- ❌ FAIL：Apple 公開網頁 OAuth 仍回 `Invalid client id or web redirect url`；Build 27 使用原生 Apple 登入，但尚未以真帳號完成 ID token 端到端驗收。
...
- ❌ FAIL：Build 27 尚未由 Edward 實際點「拍照」確認相機畫面可開啟；自動檢查與權限字串已通過，但不能代替實體相機操作。
...
- ❌ WAIT：Build 27 已上傳 App Store Connect，但尚未確認 Apple 處理完成、TestFlight 可選用，也尚未產生 Xcode Privacy Report。

## 上傳決策

目前 **已上傳、不可送審**。Build 27 的工程驗證、正式簽章、IPA 防漏與 App Store Connect 收件均通過；但 Apple 尚在處理，而且 Voice 查資料新版、真人語音、真實登入／Gateway 通話、APNs、拍照、Sandbox 金流、資料權利與審核元資料仍有未通過項目。
...
5. 等待 Apple 完成 Build 27 處理，確認 TestFlight 可選用；生成 Xcode Privacy Report 並複核第三方 SDK manifest。
