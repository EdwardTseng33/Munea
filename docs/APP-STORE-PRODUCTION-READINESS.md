...
| 候選 IPA | ✅ 已匯出；58,862,309 bytes，SHA-256 `9d7754201980d27478602d9ea0457375b9f17344bc515c61c9b9895201d8a8dc` |
| 候選來源 | ✅ PR #91／`42748d8`；基於最新 `main@a9f23ea` |
| App Store Connect | ✅ 2026-07-16 02:11（Asia/Taipei）上傳成功；Apple 處理中 |
| iOS 最低版本 | iOS 15 |
...
- ✅ PASS：`1.0.23 (Build 28)` 開發版已覆蓋安裝到 Edward iPhone 15 Pro；手機回讀版本通過，開發 profile 設定為 TEST、Pro、1,000 點與家人假資料。Mac 自動啟動因手機鎖定被 iOS 拒絕，需 Edward 解鎖後手動開啟。
- ✅ PASS：正式 App 強制走 Call Control；只有腳本產生的 Edward 開發包可直連，Call Control 10 項契約與 Release 防漏均通過。
- ✅ PASS：Build 28 Release Archive 與 App Store IPA 已在 Mac 重建；正式 IPA 沒有測試帳號、假資料、自動登入或開發直連，並驗證 production APNs、`get-task-allow=false`、簽章、版本／Build、Bundle ID、`UIDeviceFamily=[1]`、相機／照片用途說明、HealthKit、Apple 登入 entitlement、Privacy Manifest、東京設定與最新 Web 資源。
- ✅ PASS：Build 28 已由 Xcode 正式上傳 App Store Connect；Apple 回執為 `Upload succeeded`，目前等待後台處理完成。
...
- ❌ FAIL：Build 28 的語音契約與固定探測通過，但 Edward 尚未完成真人 10 分鐘長聊，不能宣稱斷字已解決。
- ❌ FAIL：Build 28 的低噪音／插話契約通過，但 Edward 尚未完成真人五組靜音與五次插話；不能以自動結果取代真機 PASS。
...
- ❌ FAIL：Apple 公開網頁 OAuth 仍回 `Invalid client id or web redirect url`；Build 28 使用原生 Apple 登入，但尚未以真帳號完成 ID token 端到端驗收。
...
- ❌ FAIL：Build 28 尚未由 Edward 實際點「拍照」確認相機畫面可開啟；自動檢查與權限字串已通過，但不能代替實體相機操作。
...
- ❌ WAIT：Build 28 已上傳 App Store Connect，但尚未確認 Apple 處理完成、TestFlight 可選用，也尚未產生 Xcode Privacy Report。
- ❌ FAIL：App Store Connect 的 App Privacy 尚未填完；Munea 涉及帳號、健康、音訊與使用資料，不能選「不收集資料」。
- ❌ WAIT：必須在 Apple 處理完成後選取 Build 28，才能確認 13 吋 iPad 截圖要求已消失；舊 Build 27 的裝置支援不會被修改。

## 上傳決策

目前 **已上傳、不可送審**。Build 28 的工程驗證、正式簽章、iPhone-only IPA 防漏與 App Store Connect 收件均通過；但 Apple 尚在處理，而且 Voice 查資料新版、真人語音、真實登入／Gateway 通話、APNs、拍照、Sandbox 金流、App Privacy、資料權利與審核元資料仍有未通過項目。
...
5. 等待 Apple 完成 Build 28 處理，在版本頁選取新 Build，確認 iPad 截圖要求消失與 TestFlight 可選用；生成 Xcode Privacy Report 並複核第三方 SDK manifest。
