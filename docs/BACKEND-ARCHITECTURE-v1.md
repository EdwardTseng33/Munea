...
- `/admin/accounts`, `/admin/north-star`, `/admin/usage`, `/admin/credits`, `/admin/conversation-summaries`, `/admin/privacy-requests`, `/admin/safety-events`, and `/admin/audit-events` exist, but they are closed unless `MUNEA_ADMIN_API_TOKEN` is configured and sent in `X-Munea-Admin-Token`.
