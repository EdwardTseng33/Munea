...
|---|---|---|---|---|
| Codex（獨立 worktree `family-relay-voice`） | **AI 家庭傳話＋語音提醒成功回執**：指定收件人、下一通只播一次、送達／失敗重試；用藥與看診提醒須等 App 真正寫入後 AI 才能說成功。分支 `codex/family-relay-voice` 疊在 PR #41，待 #41 合併後再更新 main。 | `web/src/app.js`、`engine/server.py`、`engine/supabase_adapter.py`、`engine/live_voice_server.py`、`supabase/sql/015_*`、相關測試 | 2026-07-15 | 🔄 進行中（不碰其他 session 工作目錄） |
