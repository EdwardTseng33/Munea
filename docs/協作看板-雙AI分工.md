...
- **任務／來源**：獨立 worktree `codex/app-store-1.0.46-build2-post339-20260729`；`origin/main@61fc9870`（through #340）＋發版設定，GitHub 精確 packaged source `6d5eed83b76ae6c5ed5832447175b98bb10191c0`。納入 #334 就診摘要／原生 PDF、#336 通知列修正、#339 官網前門清理與 #340 LocaleContext redacted export；未帶入仍未合併的 Draft #319。
- **結果**：完整 `test:launch` exit 0，LocaleContext redacted export 10/10 PASS；本機測試 tree 與遠端 package tree 同為 `1911cecf`；正式 Archive／IPA 防漏全 PASS。最終 IPA 59,055,870 bytes，SHA-256 `c366017c14dee5c31d4e35d2cf3acdf9efe64436aa46460ab7885aafd645c316`。
...
### 2026-07-29 Codex 📱 進行中：#284 合併後最新 main 包版 1.0.46 Build 1（call-path risk；App E2E pending）

- **任務／來源**：獨立 worktree `codex/app-store-1.0.46-build1-post284-20260729`，最終基準 `origin/main@9e840e9`。已納入 #284 四語 App foundation、#314 全螢幕／用藥餐別／家人空狀態、#315 Apple 健康誠實空值、#317 登入鍵／字體視覺修正、#321 Voice／Avatar 同線回退、#326 健康連接流程與 #327 官網 SEO；同時包含通話前置並行與 Avatar ICE 候選提早送出。因此建立新成品，不沿用 1.0.45 Build 1 的 package／手機／Apple 證據。
- **預計檔案**：`package.json`、`package-lock.json`、`web/src/version.js`、`web/index.html`、Xcode project、`STATUS.md`、產品對齊文件、`docs/RELEASE-STATE.md` 與本看板。
- **目標／邊界**：對齊 `1.0.46 (Build 1)`，跑完整自動 Gate、Archive／IPA／防漏、Edward iPhone 安裝回讀、Apple upload／processing／1.0.46 版本頁選取與儲存。只改 App Store Connect 的草稿審核版本，不按「新增以供審查」，不送審、不核准、不公開發佈。
- **硬 Gate**：0 點真帳號與 505 點 production Gateway／Voice／Avatar 真人流程未完成前保持 `App E2E pending`；模擬器、CI、Archive、上傳或安裝不得替代。
- **23:03 實際結果**：#284 已合併（`fa9907f`），打包期間 #317、#321、#326、#327 又進 main，因此所有較早成品均已作廢；最終發版綁定 GitHub 可追溯 commit `e40dbd7`。完整 `test:launch`、strict iOS 1.0.46 (1)、Call Control 15/15、Avatar、四語 foundation／release gates、產品對齊、Archive／IPA 防漏 PASS；另補 `health.js` cache／IPA Gate 與 `munea.net` watchdog／canonical 測試漂移。第一次 IPA export 正確攔下「export gate 與 release manifest 語系開關互相矛盾」；修為只打包 `binaryLocalizationEnabled` 語系，英／日／西仍在真人／法規／商店素材 Gate 後，未提前對外宣稱支援。最終 IPA 59,025,985 bytes，SHA-256 `a52096d0322852f1041a53f68cfd0f68076336655e09cfa53e0840085c50fa96`。
- **手機／Apple**：Edward iPhone 15 Pro 已安裝、啟動並回讀 `1.0.46 (1)`；Development signing＋production config，無 QA fixture，packaged build identity=`e40dbd7`。App Store Connect upload 尚未發生：系統在外送此私有 binary 前要求 Edward 對精確成品再次明確核准；latest uploaded／selected 仍是 1.0.45 Build 1。
