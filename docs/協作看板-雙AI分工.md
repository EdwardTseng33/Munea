...
|---|---|---|---|---|
| Codex（獨立 worktree `family-relay-voice`） | **AI 家庭傳話＋語音提醒成功回執**：指定收件人、下一通只播一次、送達／失敗重試；用藥與看診提醒須等 App 真正寫入後 AI 才能說成功。PR #42 疊在 PR #41，待 #41 合併後再更新 main。後續「泛化 APNs 通知 → 首頁待聽／看文字 → 指定 relay 進聊聊 → 已聽／回覆」已落成[開發計畫](家庭傳話-通知到AI轉達-開發計畫-2026-07-15.md)；待 #42 整合後只開一條後續實作 branch，不拆零碎 PR。 | `web/src/app.js`、`engine/server.py`、`engine/supabase_adapter.py`、`engine/live_voice_server.py`、`supabase/sql/015_*`、相關測試；後續新增 push service／iOS push files | 2026-07-15 | ✅ 傳話底座程式／launch test 完成；✅ APNs＋App 內閉環施工計畫完成；🔄 PR #42 待整合與部署 |
