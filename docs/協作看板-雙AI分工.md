...
- **來源基準**：最新 `origin/main@aedca6c`；PR #29 點數週期規則與 PR #23 語音開場穩定／台語安全閘均已合併，GitHub CI 與本機 `test:launch`、`smoke:no-api`、`release:check` 已通過。
- **分支／worktree**：`codex/package-1.0.5`、`/private/tmp/munea-package-1.0.5`；不碰共享 dirty `main`，目前沒有其他開啟中的 PR。
- **版本／檔案範圍**：升為 `1.0.5 (Build 10)`；預計修改 `package.json`、`package-lock.json`、`web/src/version.js`、`scripts/enable-ios-development-profile.mjs`、`ios/App/App.xcodeproj/project.pbxproj`、Capacitor 產生的 iOS Web 資源、`STATUS.md`、`docs/APP-STORE-PRODUCTION-READINESS.md` 與本看板。
- **包版方式**：需要 Capacitor sync、Mac/Xcode arm64 真機建置、Edward 開發 profile 覆蓋安裝，以及獨立 Release Archive／App Store IPA；不部署 Voice／Avatar／Gateway／Cloud Run，不把語音 canary 升正式流量。
- **驗收邊界**：自動檢查、版本、簽章、測試資料隔離、安裝與啟動逐項 PASS／FAIL；10 分鐘長聊、30 秒靜音五組、Google／Apple 真登入與實際拍照仍需 Edward 直接操作 iPhone，未實測前維持 ❌ FAIL／待驗。
