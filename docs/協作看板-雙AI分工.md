...
### 2026-07-15 Codex 🔄 開發中：Supabase 正式資料庫雪梨 → 東京搬遷

- **負責範圍**：建立東京 Supabase Pro 新專案，將現有雪梨專案 `uhmpmystjjdqqxlpsthc` 的 Database、Auth、Storage 與完整 migrations `001–013` 搬入新專案。
- **分支／worktree**：`codex/supabase-tokyo-migration`、`E:\Claude\Munea-worktrees\supabase-tokyo-migration`；基準為 `origin/main@5e27a10`，不碰共享 dirty `main`。
- **驗收範圍**：逐項比對資料表筆數、Auth 登入、RLS、RPC、健康資料、AI 記憶、訂閱／方案與點數帳本；先以新專案 Canary 驗證，不直接切正式流量。
- **安全 Gate**：雪梨專案保留且只讀，不刪除、不停用；東京正式 URL／publishable key／service role key 尚未切入 App、Brain、Voice、Gateway 或 Cloud Run。切換前必須先回報新 project ref、驗證結果與可執行回復方案。
- **明確避讓**：不修改或操作 RunPod、GLOWS、Avatar、GPU 併發、Gateway controller 與目前正式聊聊服務。
- **目前狀態**：正在盤點 migration 順序、雪梨資料量、Auth／Storage 物件與 Supabase 帳戶可建立區域；完成 Canary 後再更新本節為完成。
