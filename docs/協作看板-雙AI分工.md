...
- **驗證進度**：顯示／發音雙向單元測試 7/7 通過；Gemini Live 原始輸出連續 3 次均為「咖紮綑」，Cloud Run Voice 實測字幕仍顯示「卡早捆」。正式 Voice `00022` 與 Brain `00046` 已先經零流量 canary 再升為 100%，另補上 Live 字幕偶發中文字間空格的詞條級整理。
