...
|---|---|---|---|---|
| Codex（獨立 worktree `notification-platform-p0`） | **通知平台 P0 正式施工**：在 PR #42 上疊分支，先完成可靠事件／裝置 token／outbox，再接 APNs、iOS 權限與導頁、本機用藥排程 v2、通知收件匣。維持單一實作 PR，不碰其他 session 的 main 或 worktree。完整範圍見[通知系統 P0 計畫](通知系統-P0-開發計畫-2026-07-15.md)。 | `supabase/sql/016_*`、`engine/notification_service.py`、`engine/apns_service.py`、`engine/server.py`、`engine/supabase_adapter.py`、通知測試、`ios/App/App/` 通知相關檔；暫不修改 `web/src/app.js` | 2026-07-15 | 🔄 Batch B：資料層、後端事件 API 與 outbox 開發中；依賴合併順序仍為 PR #41 → #42 → 本 PR |
