...
|---|---|---|---|---|
| Codex（獨立 worktree `family-relay-voice`） | **AI 家庭傳話＋語音提醒成功回執**：指定收件人、下一通只播一次、送達／失敗重試；用藥與看診提醒須等 App 真正寫入後 AI 才能說成功。PR #42 疊在 PR #41，待 #41 合併後再更新 main。後續傳話專屬流程見[家庭傳話計畫](家庭傳話-通知到AI轉達-開發計畫-2026-07-15.md)；遠端 APNs、本機排程、權限、精準導頁、收件匣、邀請、漏服與健康異常已統一成[通知系統 P0 計畫](通知系統-P0-開發計畫-2026-07-15.md)。Gate A 完成後只開一條通知平台 branch，不拆零碎 PR。 | `web/src/app.js`、`engine/server.py`、`engine/supabase_adapter.py`、`engine/live_voice_server.py`、`supabase/sql/014_*`／`015_*`、相關測試；後續新增 push service／iOS push files | 2026-07-15 | ✅ 傳話底座程式／launch test 完成；✅ 全通知 P0 施工計畫完成；🔄 PR #42 待整合與部署 |
