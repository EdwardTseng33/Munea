...
- **來源／隔離**：以已合併 PR #54、#57、#58 的 `origin/main@e02fa6b` 建立獨立 worktree `/private/tmp/munea-tokyo-production-rollout`、分支 `codex/tokyo-production-rollout`；不修改或清理共享 dirty main。
- **資料環境目標**：App／Web、Gateway 與正式 Brain 凡讀寫正式使用者資料都必須指向東京 `fespbkdwafueyonppzwq`；雪梨專案、舊 revision 與舊 secret version 完整保留作回復。APNs 為 Apple 全球服務；Voice、GLOWS／RunPod 不因 Supabase 搬遷而改區域或主機。
...
- **獨立 Gate**：東京 migrations 001–015 依搬遷報告維持 PASS；通知 migration 016、APNs 金鑰／真推播、Google／Apple 真登入、拍照、StoreKit 與全語音真人體驗仍逐項驗證，不因部署或自動測試自動標綠；本輪不自動上傳 App Store Connect。
