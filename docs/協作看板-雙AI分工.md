...
### 2026-07-28 Claude/城堡 🛠️ M1 PR-4a：就診摘要組裝層（純函式・後端・未接線）

- **Edward 方向性修正（重要，已內化）**：「**我們不是要增加子女的負擔而是要降低，所以不會有也不需要子女代為設定**」。我先前引用專案文件那句「做整理工的是子女」當設計原則是**錯的**——那是**問題描述**，不是目標狀態。沐寧存在的目的是**取代**那份整理工，不是給子女一個做整理工的工具。⇒ 已移除規劃中的「子女幫忙加口袋問題」。手動加一題保留，但那是**長輩自己**加。**連帶**：AI 記錄這條路**沒有人工後援**，它必須夠準——這讓 PR-3 標注的「App E2E 未驗證」風險**更重，不是更輕**。
- **Edward 拍板的設計**（本輪確認）：①看診當天出現在每日任務，點開＝就診摘要 ②設定頁可選匯出期間 **7/14/30/60** 四選項 ③展開後右上角匯出鈕（PDF 等）。
- **查證發現（改變架構的兩件事）**：
  - ⚠️ **既有 `#reportModal`「回診摘要」是死的**：`#reportBtn` 全專案不存在→從未綁定→面板打不開，內容全是寫死假資料。⇒ **升級它，不另開一個**（版型與分享管線可用）。
  - ⚠️ **App 端拿不到症狀記憶**：`memory_items` 純後端。⇒ 摘要必須**後端組裝**、前端存快照。這正好接上先前的「小抄要存檔」決定：前一天後端組好→存快照→**診間離線可開**，且不受記憶淘汰影響。
  - ✅ `HEALTH_LOG_RETENTION_DAYS = 365` → 7/14/30/60 四個期間量測資料全支援。
  - ✅ 點任務卡目前＝打勾完成（`toggleTask`），會與「點開摘要」搶點擊；`mood` 任務已有「點了跳頁」的先例可循。
- **本次產出**：`engine/visit_summary.py`（純函式、**不呼叫 AI**、零新依賴）＋`engine/test_visit_summary.py`（**23 項全過**），掛進 `test:launch`。
  - 繼承 `health_context` 三條鐵律，另立三條專屬：**④來源必須標記**（自述 vs 量測，可信度天差地遠，醫師要能一眼分辨）、**⑤只並列不連線**（同日症狀＋數據只能並排，寫成因果＝判讀＝紅線）、**⑥截斷要誠實揭露**（超過一頁上限 12 條就明說「另有 N 筆未列出」，不可悄悄截斷）。
  - 症狀文字**原樣照搬不改寫**（改寫＝替長輩發言）；信心 <0.6 不收；非 health_context 型記憶（興趣／親屬／情緒）**一律不得混入**（防洩漏無關私事給醫師）。
  - 用藥改為**整段期間**彙總（既有 `summarize_medication` 只算當天）；**不出百分比**（百分比像評分，評分是判定）；漏 3 次以上才進時間軸。
  - 一頁上限挑選優先序 **症狀 > 用藥 > 量測**（症狀是醫師問診問不出來的東西，稀缺位置留給它）。
- **測試抓到我自己兩個錯**：①用藥測資跨 14 天但視窗是「今天＋前 13 天」，**程式對、我的測資錯** ②我寫的免責句「不是醫學**正常值**」踩到自己設的紅線禁字——**選擇改寫句子而非給它豁免**，豁免會開後門讓人以 「免責」形式偷渡判定字眼。
...
### 2026-07-28 Claude/城堡 🔧 PR-4e：補上常駐入口（Edward 抽查抓到——實作與我描述的不一致）

- **Edward 抽查**：「確認一下妳的設計觸發功能在哪裡看得到入口」。追程式後確認**我實作的與我先前描述的不一致**，是真缺陷：
  - 實際上**唯一**能打開摘要的是首頁「今天一起完成」的看診卡，而 `visitToday()` **只在當天有排定看診時**才回傳資料 ⇒ 卡片才會出現。
  - 設定頁那一列**只切換天數、不會打開摘要**——它是設定，不是入口。
  - **狀態頁入口我根本沒做**（先前規劃講了「狀態頁常駐入口＝主入口」，實作漏了）。
  - ⇒ 沒設看診提醒、想前一天先準備、平常想看看整理成什麼樣——**通通進不去，功能九成時間是隱形的**。
  - 諷刺點：我才寫過一支測試專門防「面板存在但沒有入口」的舊 bug（`#reportBtn` 不存在），結果自己犯了同一類的錯——**入口存在，但幾乎沒人碰得到**。「面板打得開」與「使用者找得到」是兩件事。
- **修正**：
  - `web/index.html` 狀態頁新增 `#visitSummaryCard` 常駐入口卡（照用藥卡同一形狀；狀態頁＝「我的身體」，摘要正是這些東西的總結）。副標／膠囊會顯示「已經記下 N 個要問醫生的問題」，給一個點進去的理由。
  - 設定頁那一列改為**點右邊天數＝改預設期間、點列的其他地方＝打開摘要**（設定頁其他每一列點下去都會開一個東西，只有這列不開很怪）。
  - `refreshVisitSummaryCard()` 接上 5 個重繪點（加題／刪題／看完醫生了／開 App／設定頁初始化）。
  - 三個入口的 `visit_summary_opened` 都帶 `source`（`daily-task`／`status-card`／`settings`）——否則 H1 分不出哪條路有人用。
- **測試**：新增第①b 組「入口要**碰得到**、不只是**存在**」——要求至少兩個**不依賴當天有排看診**的常駐入口，且開啟來源可追蹤。**96 項全過**；完整 `npm run test:launch` **exit=0**。
- **⚠ 仍未驗**：真機 E2E 與 Xcode 編譯（環境無 Xcode）。

### 2026-07-28 Claude/城堡 ✂️ 入口定案：恰好兩個（Edward 判定狀態頁常駐卡過度曝光）

- **Edward 定案**：「①首頁當天出現看診提醒任務時可以點開來看 ②設定頁『健康資料與安全』有一欄位可以打開來看。**畢竟這不是成天需要看的**。」
- **判斷成立，已照辦**：我上一輪為了補「功能九成時間隱形」的缺陷，加了狀態頁常駐卡——**修過頭了**。狀態頁是每天在看的「我的身體」頁；就診摘要一年用不到幾次，常駐在那裡 360 天都是噪音。**已撤除**（HTML 卡片 15 行、點擊處理、`refreshVisitSummaryCard()` 與其 6 個呼叫點全部清乾淨，無殘留參照）。
- **最終入口恰好兩個**（`grep` 實測確認）：
  - `openVisitSummary('daily-task')`——首頁「今天一起完成」的看診任務卡（時機驅動）
