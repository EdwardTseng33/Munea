...
|---|---|---|---|---|
| Codex（獨立 worktree `family-relay-voice`） | **AI 家庭傳話＋語音提醒成功回執**：指定收件人、下一通只播一次、送達／失敗重試；用藥與看診提醒須等 App 真正寫入後 AI 才能說成功。PR #42 疊在 PR #41，待 #41 合併後再更新 main。 | `web/src/app.js`、`engine/server.py`、`engine/supabase_adapter.py`、`engine/live_voice_server.py`、`supabase/sql/015_*`、相關測試 | 2026-07-15 | ✅ 程式／完整 launch test 已完成；🔄 PR #42 待整合與部署 |
...
### 2026-07-15 Codex / Mac ⚠️ 開發後：東京 App `1.0.10 (Build 15)` 已裝手機，Gateway 待正式切流量授權
...
- ⚠️ **Gateway 正式切換待授權**：100% 流量提升因需要 Edward 明確批准而未執行；目前線上 Gateway 仍是雪梨 revision，東京 Canary 不接正式流量。RunPod／GLOWS 主機、模型、卡片與流量完全未修改。
