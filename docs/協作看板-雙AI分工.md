...
### 2026-07-15 Codex / Windows ✅ 開發後：正式上線前 API 與 Infra 健康排查

- **任務**：驗收 Web／iOS App、Brain、Voice、Gateway、GLOWS、RunPod、Cloud Run 與東京 Supabase；排查錯誤率、重複 API 呼叫、冷啟動與正式環境指向。
- **已確認 P0**：iPhone 對 Brain 的跨來源 `OPTIONS` 預檢收到 501，造成 `/family/state` 等實際請求無法送出；Brain Cloud Run 仍指向雪梨 Supabase，與 App／Gateway 的東京正式環境不一致。
- **本輪檔案**：`engine/server.py`（正式 origin allowlist／preflight）、`web/src/app.js`（同步 single-flight、背景停拉）、對應測試、部署設定與本看板。
- **不操作範圍**：不碰 iOS 視覺與包版、不更動 GLOWS／RunPod GPU 模型或槽數、不刪雪梨回復庫。當下沒有開啟中的 PR，已自最新 `origin/main` 建立獨立 worktree／branch。
- ✅ **修復與驗證**：Brain 加入嚴格 origin allowlist 與 `OPTIONS`；iPhone `capacitor://localhost` canary 預檢由 501 修為 204，陌生來源維持 403，實際錯誤回應也帶正確 CORS。App 家庭同步加入 single-flight、30 秒冷卻、同值抑制、同鍵合併與背景停拉，避免重複 init／健康同步造成暴量請求。完整 `test:launch` 全綠。
- ✅ **正式資料與服務**：Brain 已由東京搬遷工作更新為 `fespbkdwafueyonppzwq`，管理讀取回報 `backend=supabase`；Gateway、GLOWS 主卡、RunPod controller／monitor 均健康，語音儲值後真人探針恢復（約 0.22 秒連線、1.1 秒首音）。
- ⚠️ **仍待外部設定**：Brain `/healthz/` 顯示 APNs production key／team／private key 未配置；這不阻擋聊聊，但 App Store 上線前若要提供遠端照護通知，必須在 Apple Developer 建立金鑰後掛 Secret Manager。Voice／Brain 目前仍可縮到 0，首通冷啟動風險另列上線決策。
