...
def test_he_asks_she_says_she_doesnt_know():
    ...
    2026-07-31 修：原本這裡釘的是「你查不了，就老實說」——那句寫於 2026-07-17，
    當時查詢還沒上正式機（8-9 秒又常失敗）。查詢 7/27 上線（1.3 秒）之後，
    那句變成「明明查得到卻叫她裝傻」，已由 Edward 重新定調拿掉。
    **真正不能掉的是防掰店名那一層**，這裡改釘它——不是釘那句過期的指令。
    """
    inst = instructions_with_location()
    check("查不到就老實說、不准憑印象掰店名", "不准憑印象掰店名" in inst)
    check("有查詢能力時要真的去查（不裝傻）", "就真的去查" in inst)
    check("只講查到的真店名真地點", "只講查到的真店名真地點" in inst)
