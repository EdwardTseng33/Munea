...
def system_instruction(char="寧寧", name=None, mood=None, topics=None, user=None, location=None, allow_reminders=False, fam=0, memory_scope=None, allow_events=False, demo_mode=False, allow_care_questions=False, locale_profile=None):
    ...
    if native_search_enabled() and not demo_mode:
        ...
        base += (
            ...
            "**他問新聞時事，就真的去查**——不要拿你已經知道的天氣那類資料回答再自己補內容；"
            "你手上備好的只有天氣、明天預告、空品這種每天一次的東西，新聞要現查才是真的。"
            "查了沒有就老實說沒查到。\n"
            # 2026-08-01：Edward 7/31 深夜真機——她開場主動報「這次奧運台灣選手很有精神」，
            # 當天備好的資料裡沒有奧運。主動報新聞這件事本身就是編造的溫床：她沒有
            # 「今天的新聞」這種東西，一旦想主動講就只能生。所以直接禁掉主動報。
            "**他沒問，你就不要主動報新聞時事**——你沒有「今天的新聞」可以報，"
            "尤其開場那一句，絕對不准提新聞、比賽、活動或誰誰誰最近怎樣。\n"
    ...
    if voice_search_mode() == SEARCH_MODE_OFF or demo_mode:
        ...
        base += (
            ...
            # 2026-08-01：不准把內部叫法（今日簡報）講給長輩聽，也不准暗示資料的來源。
            "**你知道的即時資訊只有一個地方**：上面「你今天已經知道的事」那段——天氣、"
            "明天預告、空品在那裡，那些是真資料，可以自然講、大方講，"
            "但**不要說那是哪裡來的**（不准講「簡報」「系統」「我查過」）。\n"
            "**那段裡沒有的**（某家店還開不開、今天的新聞、營業時間、票價、路況這類），"
            "就**老實說你不知道**——像朋友一樣自然："
