...
def system_instruction(char="寧寧", name=None, mood=None, topics=None, user=None, location=None, allow_reminders=False, fam=0, memory_scope=None, allow_events=False, demo_mode=False, allow_care_questions=False, locale_profile=None):
    ...
    # 情境段落（設提醒／記行程／查詢／熟識度……）2026-08-13 從這支程式搬進 persona 檔案。
    # 搬家理由：這批原本是寫死的中文字串、佔說明書四分之一，而且四種語系全部拿到中文版
    # （實測英文與西文說明書各夾 2,835 個中文字）。放進檔案外語版才跟得上。
    _SECTIONS = eng.voice_sections(_book_locale)
    ...
    if native_search_enabled() and not demo_mode:
        ...
        base += (
            _SECTIONS["search-health-exception"]
        )
    if demo_mode:
        base += (
            _SECTIONS["demo-visitor"]
        )
    try:
        if demo_mode:
    ...
    base += (
        "\n" + _SECTIONS["call-contract"]
    )
    # 熟識度分寸貫穿整段對話（不只開場）：越不熟越收斂、越熟越自在（Edward 2026-07-12）
    if fam < 1:
        base += _SECTIONS["familiarity-new"]
    elif fam < 3:
        base += (
            _SECTIONS["familiarity-mid"]
        )
    else:
        base += (
            _SECTIONS["familiarity-close"]
        )
    if native_search_enabled() and not demo_mode:
