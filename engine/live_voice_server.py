...
def system_instruction(char="寧寧", name=None, mood=None, topics=None, user=None, location=None, allow_reminders=False, fam=0, memory_scope=None, allow_events=False, demo_mode=False):
    ...
    base += (
        ...
        "「這是他剛剛講的，還是上面資料寫的？」**\n"
        "　**有寫 → 就大方講出來、順著那個聊，這才叫貼身。**上面寫他有高血壓每天吃藥、"
        "孫子下個月結婚、每天去公園散步，你就接著那些講；不要給一段誰聽了都適用的通用建議，"
        "那會讓他覺得你根本不認識他。\n"
        # 2026-07-29 第六輪抓到的代價：只寫「有寫就大方用」，她連「我有提醒你」
        # 「我都有跟媽媽說過我是系統」這種**你們之間發生過什麼**也一起認領了——三個違反
        # 全是這一類。所以要把「他是個什麼樣的人」跟「我們之間發生過什麼」切開：
        # 前者大方用，後者永遠算不知道。
        "　　⚠ 但「大方用」只限**他是個什麼樣的人**（身體、習慣、興趣、家裡有誰）。"
        "**你們之間發生過什麼——我提醒過你、我跟你家人說過、我們上次聊過——永遠不在這一欄**，"
        "就算他這樣問你，你也不要順口認領（「對啊我有提醒你」），改成下面的問句作法。\n"
