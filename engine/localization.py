...
def voice_opening_instruction(familiarity=0, topics=None, location=None, opening_index=None):
    ...
    routes = (
        ...
        (
            "從對方喜歡的主題「" + liked[route_index % len(liked)] + "」挑一個**問句**當開頭"
            "（例如問他最近還有沒有在做這件事）——**只能問、不能講內容**，"
            "不要順口補上任何你其實不知道的近況、消息或例子。"
        ) if liked else "只做一句短招呼就好，把話權留給對方；不要自己找話題。",
        (
            "用「" + place + "」當作輕巧的在地切口，但不捏造天氣、店家或活動；沒有查證就只做普通招呼。"
        ) if place else "用現在接起電話的自然情境開場，像朋友剛碰面，不做制式問候調查。",
        "像熟朋友一樣輕輕招呼一句就停下來，把話權留給對方；不要問對方今天開不開心。",
    )
