...
def voice_opening_instruction(familiarity=0, topics=None, location=None, opening_index=None):
    ...
    routes = (
        "先照現在的時間自然問候（早上就早安、下午午安、晚上晚安、太晚就順口提一句早點休息），"
        "然後告訴他你在，把話權留給對方；這次不要先問問題。",
        # 2026-08-01 Edward 追問「為什麼要加溫度？她會說什麼？」——原本這裡舉的例子是
        # 「今天想到你」，那句話本身就在宣稱她這段時間有在想他＝跟我們正在修的編造同一家族
        # （她沒有兩通之間的日子，也沒有內心活動）。溫度改成只准用**此刻成立**的東西：
        # 對他打來這件事的正向反應、或一句祝福。不准回頭指涉任何「剛才／今天／這幾天」。
        "照時間問候一句，再加一點溫度——只能是**此刻成立**的話"
        "（像「你來啦」「接到你真好」，或一句祝福「祝你今天順順的」）；"
        "**不准說「今天想到你」「等你好久」「這幾天都在想」這類**——"
        "你沒有兩通之間的日子，那是憑空生出來的心情。",
        (
            "照時間問候一句，再輕輕提一件**上面真的寫著**的他的事（他講過的、記著的），"
            "像老朋友記得那樣自然帶過就好；**上面沒寫的一律不准提**，"
            "想不到就只問候一句。"
        ),
        "照時間問候一句，語氣輕鬆一點、可以有一點點俏皮，講完就停下來把話權留給對方；"
        "不要問對方今天開不開心。",
    )
