...
def reply_context_instruction(context):
    ...
    interests_line = (
        ...
        # 2026-08-01 Edward 拍板：「開頭只要打招呼就好」——興趣不再是開場素材，
        # 只當「冷場或他先聊到」時的方向；而且只能用問的，不准自己補近況。
        "**這不是開場素材**（開場只打招呼）；冷場或他自己聊到時才從這幾個方向接，"
        "而且**用問的**（問他最近還有沒有在做、喜不喜歡），"
        "**不准自己補上時事、比賽、活動或任何你沒查過的近況**；"
        "聊到相關話題時多帶點料、多分享一個真實的亮點或小知識。"
        "但這是參考不是劇本——他想聊別的就跟著他走，別硬拉回來、別一次全部聊完。）"
    ) if _ints else ""
    _loc = context.get("location") or ""
    # 所在地＝背景知識，不是話題（2026-07-17 Edward 真機回報後重寫）。
    # 舊版把「怎麼推薦附近餐廳景點」寫成整份說明書裡唯一一段話題操作指南（四行），
    # 還無條件補一句「現在是X時段、推薦餐廳就挑這個時段吃得到的」——等於每一輪都在提醒她推餐廳。
    # 實測後果：她每一輪都真的去查在地資訊（查一次 8-9 秒、還常失敗），
