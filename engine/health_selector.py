...
def render(topic_id, user_text="", profile=None, hour=None, locale=None):
    ...
    # 2026-08-12 Edward 親點：「一次一兩句短話」那條鐵律已經作廢，改成**她自己判斷該長該短**。
    # 這段開頭原本還寫著「不要一次全講完、保持一兩句短話」——跟 8/6 改過的口語風格書
    # （「短是預設不是硬上限；健康說明先講結論、再講二到四個必要的重點」）直接打架，
    # 而且它是後面才貼上去的、通常壓過前面。等於我們挑好三張，又叫她不要講完。
    # 現在只給素材與順序，話量交還給她——但「先接住情緒」「不推薦品牌」「不判斷用藥」照舊。
    head = (
        "（因人挑選的方案・這一輪他聊到這個困擾，下面是**按他的狀況挑好的**方案。"
        "先接住他的情緒，再照順序用——主推先講。"
        "**話量你自己判斷**：他只是抱怨一句就短短接住；真的在問怎麼辦，就先講結論、"
        "再講真正必要的重點，分成好聽懂的小段、一次推進一件事；他插話就停，下一輪接他新說的。"
        "沒用到的方案留著下次，不必硬塞進同一輪。"
        "絕不推薦品牌、不做藥物判斷："
    )
