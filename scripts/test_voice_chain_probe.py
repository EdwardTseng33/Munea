...
def main():
    ...
    production_canary = "wss://prod-0810-161906-1d3d6d6---munea-voice-fiu65jd4da-de.a.run.app"
    assert probe.validate_voice_canary_url(production_canary) == production_canary
    for unsafe in (
    ...
    assert "VoiceCanaryUrl must be a query-free wss://canary-*, wss://stg-* or wss://prod-*" in wrapper
    assert "Run serving production Voice reconnect probe" in wrapper
    assert "--media-check" in wrapper
