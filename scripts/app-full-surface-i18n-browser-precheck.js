...
const REPORT_PATH = path.join(OUTPUT_DIR, 'full-surface-standard-local-browser-precheck.json');
...
function sourceLanguageSamples(locale, text, leafTexts, zhCatalog) {
  ...
  if (locale === 'en' || locale === 'es') {
    return [...new Set(text.match(/[\u3400-\u9fff\uf900-\ufaff]{2,}/gu) || [])].slice(0, 30);
  }
...
async function stateMetrics(page, surface, locale, zhCatalog) {
  ...
  const metrics = await page.locator(anchorSelector).evaluate((element, state) => {
    ...
    const visibleViewportLeafTexts = [...element.querySelectorAll('*')]
      ...
      .map((text) => text.trim())
      .filter(Boolean);
    const visibleViewportText = visibleViewportLeafTexts
      .join('\n')
      .trim();
  ...
  const sourceLeaks = sourceLanguageSamples(
    ...
    metrics.visibleViewportLeafTexts,
    zhCatalog,
  );
  return {
    ...metrics,
    sourceLanguageSamples: sourceLeaks,
    translationResult: sourceLeaks.length ? 'fail-source-language-visible' : 'pass',
    layoutResult: (
      metrics.horizontalOverflowPixels === 0
      && metrics.documentHorizontalOverflowPixels === 0
      && metrics.overflowingChildren.length === 0
    ) ? 'pass' : 'fail-horizontal-overflow',
    visibilityResult: (
...
async function main() {
  ...
  const report = {
    schema: 'munea.app-full-surface-standard-local-browser-precheck.v1',
    ...
    scope: {
      ...
      captureProfile: 'iphone-standard',
      expectedScreenshots: SURFACE_MANIFEST.surfaces.length * LOCALES.length,
    ...
    failures: [],
    limitations: [
      'This is a 38-state x 4-locale standard-viewport local browser precheck, not exact installed-iPhone release evidence.',
      'The small-iPhone and Dynamic Type Large profiles remain part of the exact-build installed-iPhone 456-screenshot release gate.',
      'No production, staging, StoreKit, Gateway, Voice, Avatar, Supabase, or App Store Connect service was exercised.',
    ],
  };
  const zhCatalog = catalog('zh-TW');
  ...
  try {
    for (const expected of LOCALES) {
      ...
      for (const surface of SURFACE_MANIFEST.surfaces) {
        ...
        const metrics = await stateMetrics(page, surface, expected.locale, zhCatalog);
        const screenshotPath = path.join(
          OUTPUT_DIR,
          `${expected.locale}__${safeFilePart(surface.state)}__iphone390x844.png`,
        );
