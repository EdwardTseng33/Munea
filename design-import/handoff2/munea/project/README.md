...
## 元件索引
- `MuneaDS.Home` — 首頁（寧寧陪伴卡＋今天三件事＋幫你留意輪播）
- `MuneaDS.Chat` — 聊聊頁（寧寧全屏陪伴通話（人像滿版＋字幕＋通話控制列））
- `MuneaDS.Status` — 狀態頁（安心存摺合體卡＋四數據磚＋本週本月圖表）
- `MuneaDS.Family` — 家人頁（全家健康圈＋家庭活動＋全家狀態）
- `MuneaDS.Settings` — 設定頁（帳號家庭・AI角色・提醒・訂閱點數）
- `MuneaDS.BrandTokens` — 色票與字級規範卡
